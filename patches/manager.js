...
export class ClaudeSubprocess extends EventEmitter {
    ...
    async start(prompt, options) {
        // Determine if we need stream-json input (images present)
        const hasImages = Array.isArray(options.imageParts) && options.imageParts.length > 0;
        const args = this.buildArgs(options, hasImages);
        const timeout = options.timeout || DEFAULT_TIMEOUT;
        return new Promise((resolve, reject) => {
            try {
                // Use spawn() for security - no shell interpretation
                // On Windows, use shell:true so claude.cmd is resolved correctly.
                // Prompt is passed via stdin (not as CLI arg) to avoid ENAMETOOLONG.
                ...
                // With images: use stream-json format (text + image parts as JSON).
                // Without images: write plain text (avoids ENAMETOOLONG on Windows).
                if (hasImages) {
                    const content = [];
                    if (prompt) content.push({ type: "text", text: prompt });
                    content.push(...options.imageParts);
                    const jsonLine = JSON.stringify({
                        type: "user",
                        message: { role: "user", content },
                    });
                    console.error(`[Subprocess] Sending stream-json with ${options.imageParts.length} image(s)`);
    ...
    /**
     * Build CLI arguments array (prompt is passed via stdin, not as args).
     * When hasImages=true, adds --input-format stream-json to support multimodal input.
     */
    buildArgs(options, hasImages = false) {
        const args = [
            ...
            // Prompt is written to stdin to avoid Windows ENAMETOOLONG
        ];
        if (hasImages) {
            // stream-json input format allows sending multimodal content (text + images)
            args.push("--input-format", "stream-json");
        }
        // Claude CLI requires --session-id to be a valid UUID; skip if not
...
export async function verifyClaude() {
    return new Promise((resolve) => {
        const proc = spawn("claude", ["--version"], { stdio: "pipe", shell: process.platform === "win32" });
        let output = "";
